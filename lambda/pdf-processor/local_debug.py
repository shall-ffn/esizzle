...
def mock_s3_operations():
    ...
    def mock_download_pdf_from_s3(document_id, bucket_prefix):
        logger.info(f"MOCK: Downloading PDF {document_id} from bucket prefix {bucket_prefix}")
    ...
    def mock_upload_split_to_s3(pdf_data, filename, bucket_prefix):
        logger.info(f"MOCK: Uploading {len(pdf_data)} bytes to {bucket_prefix}/{filename}")
        mock_s3_key = f"{bucket_prefix}/IOriginal/Images/{filename}"
